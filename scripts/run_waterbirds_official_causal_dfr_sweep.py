...
def _metric_row(
    *,
    ...
    return {
        ...
        "causal_dfr_nuisance_prior": str(method.get("causal_dfr_nuisance_prior", "")),
        ...
        "probe_selectivity": str(metrics.get("probe/selectivity", "")),
...
def _candidate_label(config: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"_nuis{method.get('causal_dfr_nuisance_weight', '')}"
        f"_{method.get('causal_dfr_nuisance_prior', 'mask')}"
        f"_cons{method.get('dfr_counterfactual_consistency_weight', 0.0)}"
    ).replace(".", "p")
...
def _evaluate_config(
    ...
    return evaluate(fit_method(bundle, config), bundle, config)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--baseline-config", default="configs/benchmarks/waterbirds_features_official_dfr_val_tr.yaml")
    parser.add_argument("--candidate-config", default="configs/benchmarks/waterbirds_features_official_causal_dfr.yaml")
    parser.add_argument("--dataset-path", default="data/waterbirds/features_official_erm_official_repro.csv")
    parser.add_argument("--output-csv", default="outputs/dfr_sweeps/official-causal-dfr-paired-sweep.csv")
    parser.add_argument("--output-json", default="outputs/dfr_sweeps/official-causal-dfr-paired-sweep.json")
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Optional cap on hyperparameter settings for staged long sweeps; each setting still runs all requested seeds. 0 means no cap.",
    )
    ...
    nuisance_weights = _float_values(args.nuisance_weights, float(candidate_method.get("causal_dfr_nuisance_weight", 0.0)))
    nuisance_priors = _str_values(args.nuisance_priors, str(candidate_method.get("causal_dfr_nuisance_prior", "mask")))
    consistency_weights = _float_values(args.consistency_weights, float(candidate_method.get("dfr_counterfactual_consistency_weight", 0.0)))
    ...
    with output_csv.open("w", encoding="utf-8", newline="") as handle:
        ...
        for top_k, margin, nuisance_weight, nuisance_prior, consistency_weight in itertools.product(
            top_ks,
            margins,
            nuisance_weights,
            nuisance_priors,
            ...
            for seed in seeds:
                ...
                config["name"] = f"{candidate_base.get('name', 'causal_dfr')}_{label}_seed{seed}"
